...
import { useTrainingBlock, useUpdateTrainingBlock } from '@/hooks/api/use-training-blocks';
import {
  FaArrowLeft,
  FaArrowRight,
  FaArrowUp,
...
export const EditTrainingBlock = () => {
  const { state } = useAuth();
  ...
  const updateTrainingBlockMutation = useUpdateTrainingBlock();
  ...
  // Memoize filtered and grouped exercises
  ...
  const breadcrumbs = [
    ...
    { label: 'Edit Training Block' },
  ];

  // Load training block data when it's available
  useEffect(() => {
    if (trainingBlockData) {
      const exercises = trainingBlockData.exercises.map((exercise) => ({
      ...
      setTrainingBlockBuilder({
        workout_name: trainingBlockData.workoutName,
        workout_method: trainingBlockData.workoutMethod,
        workout_type: trainingBlockData.workoutType,
        is_free: trainingBlockData.is_free ?? false,
        exercises: exercises,
        summary: {
          nl: trainingBlockData.summary?.nl ?? 0,
          totalStress: trainingBlockData.summary?.totalStress ?? 0,
          centralStress: trainingBlockData.summary?.centralStress ?? 0,
          peripheralStress: trainingBlockData.summary?.peripheralStress ?? 0,
          csBalance: trainingBlockData.summary?.csBalance ?? 0,
          patterns: trainingBlockData.summary?.patterns ?? [],
        },
      });
    }
  }, [trainingBlockData]);
  ...
  useEffect(() => {
    if (!trainingBlockId) {
      addToast({
        title: 'No training block ID provided',
        color: 'danger',
      });
  ...
  const handleUpdateTrainingBlock = async () => {
    ...
    if (hasInvalidSet) {
      setOnUpdating(false);
      addToast({
        title: 'Each exercise set must have reps less than 30 and RPE (0-10)',
        color: 'danger',
      });
      return;
    }
    ...
    try {
      ...
      await updateTrainingBlockMutation.mutateAsync(
        { id: trainingBlockId, data: updateData, tenantId: effectiveTenantId },
        {
          onSuccess: () => {
            addToast({
              title: 'Training block updated successfully',
              color: 'success',
            });
            router.push('/workout/manage-workouts');
          },
          onError: () => {
            addToast({
              title: 'Failed to update training block',
              color: 'danger',
            });
          },
        },
      ...
      addToast({
        title: 'Unexpected error during update',
        color: 'danger',
      });
    } finally {
      setOnUpdating(false);
  ...
  if (isLoadingTrainingBlock || onUpdating) {
    return (
    ...
    <PageWrapper breadcrumbs={breadcrumbs}>
      <div className="flex flex-col gap-4 p-4 rounded-md bg-backgroundSecondary border border-border">
        <div className="flex flex-col sm:flex-row justify-between gap-y-2">
          <h1 className="text-2xl font-semibold text-text">Edit Training Block</h1>
          <div className="flex justify-end gap-3">
            <Button
              variant="solid"
              color="primary"
              onPress={handleUpdateTrainingBlock}
              isLoading={updateTrainingBlockMutation.isPending}
            >
                  ...
                  <>
                    <SelectItem key="CUTTING" textValue="CUTTING">
                      <div className="flex flex-col">
                        ...
                        <div className="text-sm">Focus on fat loss and muscle preservation.</div>
                      </div>
                    </SelectItem>
                      ...
                      <div className="flex flex-col gap-1 text-xs text-text">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {typeof trainingBlockBuilder.summary?.nl === 'number' &&
                            !isNaN(trainingBlockBuilder.summary.nl)
                              ? trainingBlockBuilder.summary.nl.toFixed(2)
                              : 0}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {typeof trainingBlockBuilder.summary?.peripheralStress === 'number' &&
                            !isNaN(trainingBlockBuilder.summary.peripheralStress)
                              ? trainingBlockBuilder.summary.peripheralStress.toFixed(2)
                              : 0}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Central:</span>
                          <span className="font-medium">
                            {typeof trainingBlockBuilder.summary?.centralStress === 'number' &&
                            !isNaN(trainingBlockBuilder.summary.centralStress)
                              ? trainingBlockBuilder.summary.centralStress.toFixed(2)
                              : 0}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Total:</span>
                          <span className="font-medium">
                            {typeof trainingBlockBuilder.summary?.totalStress === 'number' &&
                            !isNaN(trainingBlockBuilder.summary.totalStress)
                              ? trainingBlockBuilder.summary.totalStress.toFixed(2)
                              : 0}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {typeof trainingBlockBuilder.summary?.csBalance === 'number' &&
                            !isNaN(trainingBlockBuilder.summary.csBalance)
                              ? trainingBlockBuilder.summary.csBalance.toFixed(1)
                              : 0}
                            %
                          </span>
                        </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-2">
                        ...
                        {/* {trainingBlockBuilder.exercises.filter((exercise) => exercise.day === day)
                          .length > 0 && (
                                  ...
                                  <CardBody className="p-3 flex flex-col gap-2">
                                    ...
                                    {/* {exercise.modifiers.length > 0 && (
                                      <div className="flex flex-col gap-2 text-xs text-danger">
                                        {exercise.modifiers.map((modifier, index) => (
                                          <div
                                            key={`${exercise.exerciseId}-${exercise.day}-${exercise.order || 0}-modifier-${index}`}
                                          >
                                            {
                                              modifiersData?.modifiers?.find(
                                                (m) => m.id === modifier,
                                              )?.name
                                            }
                                          </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            {isExerciseAdded && (
                              ...
                              >
                                {
                                  selectedDayExercises.filter((ex) => ex.exerciseId === exercise.id)
                                    .length
                                }
                              </Chip>
                            )}
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-3 bg-backgroundSecondary rounded-lg border border-border w-full">
                                      ...
                                      <div className="flex items-center gap-1 sm:gap-2 flex-1">
                                        ...
                                        <Input
                                          ...
                                          max={10.0}
                                          value={
                                            Number.isFinite(target.rpe) ? target.rpe.toString() : ''
                                          }
                                          classNames={{
                                            inputWrapper:
                                              'px-2 border-border group-data-[focus=true]:border-border bg-white',
                          ...
                          {exercise.modifiers && exercise.modifiers.length > 0 ? (
                            <div className="text-text text-sm">
                              {exercise.modifiers
                                .map(
                                  (modifier) =>
                                    modifiersData?.modifiers?.find((m) => m.id === modifier)?.name,
                                )
                                .join(', ')}
                            </div>
                          ) : (
          ...
          <ModalBody className="py-4">
            <div className="flex flex-wrap justify-center items-center gap-2">
              {Array.from({ length: 7 }).map((_, index) => {
                ...
                const isDisabled = 
                  dayList === deployModal.day || trainingBlockBuilder.exercises.filter((exercise) => exercise.day === dayList).length > 0;
                return (
                  <Button
                    key={dayList}
                    variant="bordered"
...
const EditTrainingBlockContent = EditTrainingBlock;
