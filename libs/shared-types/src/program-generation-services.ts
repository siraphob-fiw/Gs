// Service Interfaces for Intelligent Program Generation
// Defines the interfaces for all program generation services
...
export interface FrequencyAdjustment {
    exerciseId: string;
    newFrequency: number; // sessions per week
    reason: string;
}
...
export interface TemplateSetRequest {
    ...
    rpe?: number | { min: number; max: number };
    isWarmup: boolean;
    isBackoff: boolean;
}
...
//     previewProgramChanges(
//         programId: string,
//         changes: TemplateCustomizations,
...
export interface AdaptationService {
    ...
    calculateSessionStress(session: any): Promise<any>;
    flagConcerningPatterns(athleteId: string): Promise<PerformanceAlert[]>;
    reviewAdaptationHistory(athleteId: string, programId: string): Promise<AdaptationChange[]>;
...
export interface CompetitionPlanningService {
    createCompetitionPlan(request: CompetitionPlanRequest): Promise<any>;
    ...
    updateCompetitionPlan(planId: string, updates: Partial<any>): Promise<void>;
    getCompetitionHistory(athleteId: string): Promise<Competition[]>;
...
export interface CompetitionPrediction {
    ...
    predictedPlacement: { min: number; max: number; most_likely: number };
    confidence: number;
